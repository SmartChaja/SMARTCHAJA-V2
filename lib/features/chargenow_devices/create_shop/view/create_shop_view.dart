...
import 'package:smart_chaja/features/chargenow_devices/create_shop/provider/create_shop_provider.dart';
...
class _CreateShopViewState extends ConsumerState<CreateShopView> {
  ...
  Widget build(BuildContext context) {
    final createShopState = ref.watch(createShopViewModelProvider);
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: createShopState.status == CreateShopStatus.loading
                  ? null
                  : () {
                      ref.read(createShopViewModelProvider.notifier).createShop(
              ...
              child: createShopState.status == CreateShopStatus.loading
                  ? const CircularProgressIndicator()
                  : const Text('Create Shop'),
            ),
            const SizedBox(height: 16),
            if (createShopState.status == CreateShopStatus.success && createShopState.createShopResponse != null)
              Text('Success: Shop created successfully. ${createShopState.createShopResponse!.data?.toString() ?? ''}'),
            if (createShopState.status == CreateShopStatus.error && createShopState.errorMsg != null)
              Text('Error: ${createShopState.errorMsg}', style: const TextStyle(color: Colors.red)),
            const SizedBox(height: 16),
            ElevatedButton(
              onPressed: () {
                ref.read(createShopViewModelProvider.notifier).resetState();
